...
│  │  • Валидация URL                                 │ │
│  │  • Оркестрация Cache + Repository                │ │
│  │  • Координация трекинга кликов                   │ │
│  └──────────────────────────────────────────────────┘ │
└────────┬────────────────────────┬─────────────────────┘
         │                        │
┌────────▼──────────┐    ┌────────▼─────────────┐
│  Repository слой  │    │    Cache слой        │
│                   │    │                      │
│  • PostgreSQL     │    │  • Redis             │
│  • CRUD операции  │    │  • Get/Set/Delete    │
│  • Транзакции     │    │  • TTL: 1 час        │
│  • Миграции       │    │  • Graceful degrade  │
└───────────────────┘    └──────────────────────┘
         │                        │
┌────────▼──────────┐    ┌────────▼─────────────┐
│   PostgreSQL      │    │      Redis           │
